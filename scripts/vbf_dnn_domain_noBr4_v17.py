...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "Comma-separated eras to train, e.g. 2024,2025. "
            "The default 'all' trains 2022, 2022EE, 2023, 2023BPix, 2024, and 2025."
        ),
    )
    parser.add_argument("--use-source-year", action="store_true",
                        help="Add the distinct numeric source-era encoding as a trainable feature.")
    ...
    parser.add_argument(
        "--condor-log-prefix",
        default=None,
        help="Prefix for files written under <condor-workdir>/condor_logs.",
    )
    parser.add_argument(
        "--condor-env-setup",
...
# Keep every data-taking era distinct even when source_year is used as a
# scalar feature. The half-year values preserve chronological ordering while
# separating the pre/post-EE and pre/post-BPix eras.
SOURCE_YEAR_ENCODING = {
    "2022": 2022.0,
    "2022EE": 2022.5,
    "2023": 2023.0,
    "2023BPix": 2023.5,
...
def era_to_source_year(era):
    return SOURCE_YEAR_ENCODING.get(str(era), np.nan)
...
def ensure_source_year(df_in):
    ...
    if derived.isna().any():
        bad_eras = sorted(out.loc[derived.isna(), "era"].astype(str).unique())
        raise RuntimeError(f"Missing source-year encoding for eras: {bad_eras}")

    # The configured era is authoritative. Do not retain an ntuple source_year
    # that merges 2022/2022EE or 2023/2023BPix.
...
def write_condor_files():
    ...
    cwd = os.path.abspath(args.condor_workdir or os.getcwd())
    safe_output_name = "".join(
        ch if ch.isalnum() or ch in "-_" else "_"
        for ch in output_name
    )
    condor_dir_name = f"condor_{safe_output_name}"
    condor_dir = os.path.join(cwd, condor_dir_name)
    log_dir_name = "condor_logs"
    log_dir = os.path.join(cwd, log_dir_name)
    log_prefix = args.condor_log_prefix or f"vbf_dnn_domain_{safe_output_name}"
    ...
    executable_for_submit = os.path.join(condor_dir_name, "run_fold.sh")
    ...
    with open(run_sh, "w") as f:
        f.write("#!/bin/bash\n")
        # LCG setup scripts may inspect variables such as COMPILER before they
        # are defined, so enable nounset only after sourcing the environment.
    ...
    with open(submit_file, "w") as f:
        ...
        f.write(f"executable = {executable_for_submit}\n")
        ...
        f.write(
            f"output = {log_dir_name}/{log_prefix}.$(ClusterId).$(ProcId).out\n"
        )
        f.write(
            f"error = {log_dir_name}/{log_prefix}.$(ClusterId).$(ProcId).err\n"
        )
        f.write(
            f"log = {log_dir_name}/{log_prefix}.$(ClusterId).$(ProcId).log\n"
        )
        f.write(f"queue Fold in {folds}\n")

    print("[CONDOR] wrote:", run_sh, flush=True)
    print("[CONDOR] wrote:", submit_file, flush=True)
    print("[CONDOR] logs will be written under:", log_dir, flush=True)
    print("[CONDOR] first run with --prepare-folds-only, then submit:", flush=True)
    print(f'cd "{cwd}"', flush=True)
    print(f'condor_submit "{condor_dir_name}/submit_folds.sub"', flush=True)
...
def load_or_build_datasets():
    ...
    if args.input_fold_dir is not None:
        ...
        task_cached = ensure_source_year(task_cached)
        domain_cached = ensure_source_year(domain_cached)
        ...
        print("[INFO] recoded cached source_year with:", SOURCE_YEAR_ENCODING, flush=True)
    ...
    with open(f"{output_dir}/features.json", "w") as f:
        json.dump(to_jsonable({
            ...
            "source_year_encoding": SOURCE_YEAR_ENCODING,
